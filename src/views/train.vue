...
import { rotateAroundWorldAxis, XYZ_VALUE, AxesEnum, AxesVec3, getFaceColor, colors } from '@/utils/tool'
...
class CreepCube extends Base {
  ...
  constructor (config = {
    ...
    super(config.widthRatio, config.heightRatio, config.class)
    this.initDebug()
    this.initCube()
  ...
  async initCube () {
    ...
    const gutterMaterial = new THREE.MeshBasicMaterial({ color: debug.gutterColor })
    for (let i = 0; i < d; i++) {
      for (let j = 0; j < w * h; j++) {
        const index = i * w * h + j
  ...
  getMouseSite (e: MouseEvent | TouchEvent | any) {
  ...
  onMouseDown (e: MouseEvent | TouchEvent) {
    ...
    // intersects 存在长度 说明, camera与鼠标衍生的射线碰触到物体,即说明鼠标点击在object 上
  ...
  onMouseMove (e: MouseEvent | TouchEvent) {
    ...
    if (this.intersect1 && !this.isRotating && this.startPoint) {
      ...
      if (this.movePoint.distanceTo(this.startPoint) > debug.cubeSize / 2) {
...
window.cube = new CreepCube()
</script>

<template>
  <a class="github" href="https://github.com/buuing/Rubiks-Cube">github address: https://github.com/buuing/Rubiks-Cube</a>
</template>
